...
Downloads `profile-use` (macOS / Linux, x64 / arm64). The Javascript helpers shell out to it; you don't run `profile-use` directly.

## Javascript API (pre-imported in `browser-harness -c`)

```javascript
await list_cloud_profiles()
// [{id, name, userId, cookieDomains, lastUsedAt}, ...] — every profile under this API key

await list_local_profiles()
// [{BrowserName, ProfileName, DisplayName, ProfilePath, ...}, ...] — detected on this machine

await sync_local_profile(profile_name, {
                   browser: null,
                   cloud_profile_id: null,      // update an existing cloud profile instead of creating new
                   include_domains: null,       // only these domains (and subdomains); leading dot optional
                   exclude_domains: null        // drop these domains; applied before include
})
...
await start_remote_daemon("work", { profileName: "my-work" })   // name→id resolved client-side
await start_remote_daemon("work", { profileId: "<uuid>" })      // or pass UUID directly

await stop_remote_daemon("work")                           // shut the daemon and PATCH the cloud browser to stop — billing ends
```
...
for (const p of await list_cloud_profiles()) {
    console.log(`${p.name.padEnd(25)}  ${String(p.cookieDomains.length).padStart(3)} domains  ${p.id}`);
}
```
...
await start_remote_daemon("work", { profileName: "browser-use.com" });

// 2b. Sync local first. Show the options:
for (const lp of await list_local_profiles()) {
...
const uuid = await sync_local_profile("browser-use.com");
await start_remote_daemon("work", { profileId: uuid });

// 3b. Refresh that same cloud profile later (idempotent — no duplicate profiles).
await sync_local_profile("browser-use.com", { cloud_profile_id: uuid });

// 3c. Scoped: push *only* Stripe cookies into a dedicated cloud profile.
await sync_local_profile("browser-use.com", {
...
- API: `GET /profiles`, `GET/PATCH/DELETE /profiles/{id}` (paths are relative to `BU_API = "https://api.browser-use.com/api/v3"` in `admin.js`). Fields: `id`, `name`, `userId`, `lastUsedAt`, `cookieDomains[]`. `list_cloud_profiles()` wraps this.
- Name → UUID: `profileName=` on `start_remote_daemon` resolves client-side; no API change needed.
- Need the UUID for an existing profile? `matches = (await list_cloud_profiles()).filter(p => p.name === "<name>").map(p => p.id)` — then verify `matches.length === 1` before using it. Profile names are not unique; syncs create duplicates unless you pass `cloud_profile_id=`.
- Lower-level raw calls: `import { _browser_use } from './admin.js'; await _browser_use("/profiles/<id>", "DELETE")`. Pass the path *without* the `/api/v3` prefix — it's already on `BU_API`.
